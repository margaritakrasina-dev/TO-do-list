...
export default function ResetPasswordPage() {
  ...
  // 👀 Zjistíme, jestli se uživatel dostal z emailu (má aktivní session)
  useEffect(() => {
    const checkSession = async () => {
      const { data } = await supabase.auth.getSession();
      if (data.session) {
        setIsResetting(true); // zobrazíme formulář pro nové heslo
      }
    };
    checkSession();
  }, []);

  // ✉️ Fáze 1 – odeslání emailu s odkazem
  const handleSendEmail = async (e: React.FormEvent) => {
    e.preventDefault();

    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: "http://localhost:3000/reset-password", // sem Supabase přesměruje po kliknutí z emailu
    });

    if (error) setMessage(error.message);
    else setMessage("📩 Email pro obnovení hesla byl odeslán. Zkontroluj svou schránku.");
  };

  // 🔑 Fáze 2 – nastavení nového hesla
